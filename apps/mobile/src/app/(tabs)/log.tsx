...
export default function LogScreen() {
  ...
  useEffect(() => { const client = getSupabaseClient(); if (!client || !auth.session) return; void Promise.all([loadCompletedWorkouts(client), loadEvidenceHistory(client)]).then(([workouts, history]) => { setItems(workouts); setEvidence(history); }).catch((cause: unknown) => setError(cause instanceof Error ? cause.message : 'Could not load history.')); }, [auth.session]);
