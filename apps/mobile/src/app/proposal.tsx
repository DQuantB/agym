...
export default function ProposalScreen() {
  ...
  useEffect(() => {
    ...
    void loadProposalById(client, id).then((value) => {
      setProposal(value);
      setLoadState(value ? { kind: 'ready' } : { kind: 'missing' });
    }).catch((error: unknown) => setLoadState({ kind: 'error', message: error instanceof Error ? error.message : 'Could not load proposal.' }));
    ...
    <View style={styles.screen}>
      ...
      <View style={styles.statusStack}>
        {loadState.kind === 'loading' ? (
          <StatusCard busy title="Loading proposal" detail="Fetching the agent-authored plan awaiting your review." />
        ) : null}
        {loadState.kind === 'error' ? (
          <>
            <StatusCard tone="warning" title="Proposal unavailable" detail={loadState.message} />
            <Button label="Retry" variant="secondary" fullWidth onPress={() => setReloadToken((token) => token + 1)} />
          </>
        ) : null}
        {loadState.kind === 'missing' ? (
          <>
            <StatusCard tone="proposal" title="No proposal to review" detail="This proposal is no longer awaiting review." />
            <Button label="Back to calendar" variant="secondary" fullWidth onPress={() => router.back()} />
          </>
        ) : null}
      </View>
    </View>
