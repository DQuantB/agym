...
export function CaptureScreen() {
  ...
  return <Screen eyebrow="CAPTURE" title="Log reality" onBack={goBack}><ScrollView contentContainerStyle={styles.stack} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
    {!draft ? <><Text style={styles.lead}>Write the messy reality. AGYM saves your words first, then proposes uncertain structure for review.</Text><TextInput accessibilityLabel="Raw training log" multiline value={text} onChangeText={setText} placeholder="Squat 3x8@80kg; knee felt sore on set 3" placeholderTextColor={colors.muted} style={styles.raw} /><TextInput accessibilityLabel="Log date" value={date} onChangeText={setDate} placeholder="YYYY-MM-DD" placeholderTextColor={colors.muted} style={styles.input} /><Button label={busy ? 'Saving…' : 'Save raw log and review'} variant="primary" fullWidth busy={busy} disabled={!text.trim()} onPress={() => void save()} /></> : <>
      ...
      <Button label={busy ? 'Confirming…' : 'Confirm corrected result'} variant="primary" fullWidth busy={busy} onPress={() => Alert.alert('Confirm result?', 'This creates canonical user-confirmed history. Raw evidence stays unchanged.', [{ text: 'Cancel', style: 'cancel' }, { text: 'Confirm', onPress: () => void confirm() }])} /><Button label="Discard draft (keep raw evidence)" variant="secondary" fullWidth disabled={busy} onPress={() => { setDraft(null); setOriginal(null); setMessage('Draft discarded. Raw evidence remains preserved.'); }} />
