...
export function WorkoutExecutionScreen() {
  ...
  return <ScrollView style={styles.screen} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    ...
    {editor.actualData.exercises.map((exercise, exerciseIndex) => <View key={exercise.client_id} style={styles.card}>{exercise.user_added ? <TextInput accessibilityLabel={`Actual exercise ${exerciseIndex + 1} name`} style={styles.input} value={exercise.name} onChangeText={(name) => dispatch({ type: 'set_exercise_name', exerciseIndex, name })} onEndEditing={(event) => dispatch({ type: 'set_exercise_name', exerciseIndex, name: event.nativeEvent.text })} /> : <Text style={styles.exercise}>{exercise.name}</Text>}{exercise.sets.map((set, setIndex) => {
