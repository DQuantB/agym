...
**Goal:** turn the existing hosted AGym alpha into a mobile-first iOS/Android plan-to-reality layer for hybrid athletes: a signed-in user can receive a proposal from their chosen agent, execute or adapt it, capture cross-domain reality and constraints, confirm what happened, and return bounded user-confirmed context to that agent.

**Architecture:** retain the hosted Supabase/Auth/RLS/MCP system as the shared data and trust boundary. Add a native Expo/React Native client at `apps/mobile`; keep the existing Vite client as the founder/admin/browser companion during alpha. Share only schemas, API contracts, and pure domain logic after a small extraction; do not try to share React components or CSS. Preserve the current raw → uncertain draft → user-confirmed event/provenance model. AGym complements specialist trackers and the user's chosen LLM; it is not a tracker replacement or an authoritative AI coach.
...
**Explicit non-goals for this plan:** a full AI coach, autonomous training recommendations, medical advice, a broad wearable/app integration catalogue, social features, coach roster/dashboard, subscriptions/payments, proprietary/fine-tuned fitness models, or an unreviewed third-party exercise-media bundle. A future professional workflow requires separate discovery and a concierge briefing test before dashboard work.
...
```text
Claude/Hermes proposes a workout or weekly training action
→ AGym mobile shows the immutable planned baseline
→ user performs, adapts, postpones, skips, or replaces the planned activity
→ rest timer and notes reduce gym-floor friction where it is a gym workout
→ user speaks or types the cross-domain reality, constraint, or deviation
→ AGym preserves raw evidence and creates an uncertain draft only where applicable
→ user confirms/corrects without overwriting original evidence
→ user reviews/copies a bounded weekly Training Context Brief
→ their chosen agent retrieves the same provenance-labelled context
```
...
- An agent context pull and the user-visible briefing can distinguish raw evidence, uncertain drafts, user-confirmed outcomes, agent proposals, and missing/unknown information.
- AGym does not generate a training conclusion or claim that it has reconciled every tracker; the athlete remains able to inspect, correct, and decide.
...
- weekly Training Context Brief generation/copy rate;
- share of planned activities marked completed-as-planned, modified, partially completed, skipped, postponed, replaced, or unknown;
- cross-domain context capture rate (for example running, nutrition, recovery, schedule constraint) and correction rate.
...
### Phase 4 — hybrid context capture, raw evidence, and correction UX

**Objective:** replace developer-oriented JSON editing with rapid, cross-domain human capture and correction while preserving uncertainty. This phase captures the deviations, constraints, and subjective context that specialist trackers normally do not connect.
...
- Modify only if a schema gap is proven: dated migration(s) under `supabase/migrations/` plus matching RLS/provenance tests

**Steps:**
1. Implement text-first capture with source, date, optional plan/activity link, and a free-form raw note. Write raw text before attempting structured parse.
2. Support an explicit low-friction context/deviation path for non-gym reality: completed-as-planned, modified, partially completed, skipped, postponed, replaced, or unknown. A reason is optional raw user text, never an AGym inference.
3. Add draft-review cards with chips/inline fields for date, plan relationship, activity/domain where known, and uncertainty reason. Keep gym-specific exercise/set/reps/load corrections available only when the raw input supports them. Keep raw JSON behind an advanced/debug affordance if still needed.
4. Ensure confirmation creates the existing user-confirmed canonical record, never overwrites raw evidence, uncertain parse, or an immutable agent proposal.
5. Implement a simple history list that visibly labels proposal/raw/draft/canonical status and missing/unknown context; no analytics charts in this phase.
6. Add unit, UI, and two-account RLS tests for all capture transitions, including a captured context note that is linked to a plan but does not mutate it.

**Acceptance:** a user can record a non-gym context event or plan deviation in a few taps; the original wording and provenance remain visible; an agent later sees confirmed reality plus correctly labelled raw evidence without AGym generating a prescription.

---

### Phase 5 — user-inspectable weekly Training Context Brief and BYOA export

**Objective:** make the weekly context packet the first user-visible payoff for hybrid athletes before building tracker integrations.

**Files:**
- Create: `apps/mobile/src/features/briefing/BriefingScreen.tsx`
- Create: `apps/mobile/src/features/briefing/buildTrainingContextBrief.ts`
- Create: `apps/mobile/src/features/briefing/buildTrainingContextBrief.test.ts`
- Modify: `apps/mobile/app/(tabs)/briefing.tsx` or equivalent route created by the native navigation scaffold
- Modify: shared context/briefing contract only after reviewing existing web/MCP behavior and tests
- Modify only if a schema gap is proven: dated migration(s) plus RLS/provenance tests

**Briefing contract:**
- current goals/block and current proposals where available;
- planned versus actual activity, including explicit deviation status;
- confirmed cross-domain context (training, nutrition/recovery/schedule only when user-supplied);
- raw/uncertain evidence clearly separated from confirmed reality;
- constraints, upcoming commitments, and missing/unknown information;
- provenance labels and a bounded date range;
- no AGym-authored recommendation, diagnosis, readiness score, or claim of complete third-party tracker coverage.

**Steps:**
1. Specify a deterministic, bounded seven-day input/output contract that shares terminology with `get_context`; do not expose the user's whole history by default.
2. Build the pure brief generator with fixtures covering a gym-plan outcome, a modified/skipped plan item, a cross-domain raw note, a confirmed constraint, and missing data.
3. Render the brief in the mobile app with clear proposal/raw/draft/confirmed/missing visual distinctions and an edit/remove path before sharing.
4. Add copy/export using a compact portable format for Claude, ChatGPT, or another user-chosen agent. Do not require a particular LLM provider or store its credentials in the app.
5. Confirm that MCP `get_context` can return the same bounded provenance-labelled facts; align shared fields rather than duplicating divergent summaries.
6. Instrument brief generation/copy and user corrections without placing health content or full brief text in generic analytics.

**Acceptance:** a user can inspect, correct, and copy a useful weekly plan-versus-reality context packet; the same bounded evidence is available to their authorized agent; the output preserves uncertainty and does not present AGym as the decision-maker.

---

### Phase 6 — voice capture spike, then gated voice implementation
...
### Phase 7 — exercise catalogue, only after licensing decision
...
### Phase 8 — privacy, consent, release, and external-alpha gates

**Objective:** ship an invite-only TestFlight/Android internal test build only after user-data ownership, consent boundaries, and mobile-specific risks are proven.
...
3. Product processing, optional research/model-improvement consent, and future professional-sharing consent are separately described; research consent is opt-in, revocable, and never required for core product utility. Do not build a fine-tuning pipeline in this phase.
4. Microphone permission has plain-language purpose text and a no-voice fallback.
5. Two-user RLS isolation is proven for native reads/writes and queued replay.
6. A real device smoke test covers sign-in persistence, deep link, Today, execution, rest timer, offline edit/reconnect, text capture, correction, briefing copy/export, export/delete.
7. EAS/internal distribution credentials are kept outside Git/chat; builds are reproducible from documented non-secret configuration.
8. No external tester receives a claim of clinical advice, automatic diagnosis, autonomous coaching, or complete third-party tracker coverage.

---

## 6. Integration decision gate

Do not add a broad tracker catalogue or promise universal sync during mobile alpha. An import/sync is eligible for a separate scoped spike only when hybrid-athlete discovery shows that a specific source repeatedly prevents users from creating or trusting the weekly Training Context Brief.

The spike brief must state:

1. the exact user workflow and recurring manual workaround being removed;
2. the minimum fields needed for the briefing, their source provenance, and retention/deletion implications;
3. whether a user-controlled CSV/JSON import or pasted summary validates the job before OAuth/sync work;
4. API permission, cost, reliability, and support risks;
5. how imported facts remain distinct from raw self-report, agent proposals, and user-confirmed outcomes;
6. the acceptance metric: improved brief completion/trust, not integration count.

Potential sources such as Strava/Garmin activity, lifting history, nutrition summaries, or calendar constraints are hypotheses, not committed roadmap items. Do not build an integration until the founder approves the spike conclusion.

---

## 7. Recommended first four tickets

1. **Docs/status, hybrid-BYOA contract, and exercise-dataset due-diligence** — correct the Claude MCP wording, document the Training Context Brief boundary and integration decision gate, audit dataset license/fields, and document the native architecture decision.
...
4. **Hybrid context capture and weekly Training Context Brief** — capture plan deviations/non-gym constraints as raw evidence, preserve provenance, and let the user inspect/correct/copy bounded weekly context before any third-party tracker integration.

Voice and catalogue search intentionally come after the mobile workout loop, hybrid context capture, and weekly briefing are fast and reliable.

## 8. Decisions that require founder review before implementation
...
- Approve any third-party tracker import/sync only after a discovery-backed integration spike meets the §6 decision gate.
- Approve the optional research/model-improvement consent wording and data-governance design separately from product processing; no model-training pipeline is authorized by this plan.
- Do not authorize a trainer, dietitian, or coach dashboard from this plan. First complete separate professional discovery and a concierge briefing test.

## 9. Quality gates for every implementation PR
